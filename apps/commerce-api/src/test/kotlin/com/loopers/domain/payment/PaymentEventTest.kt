...
class PaymentEventTest {
    ...
    private val paymentFacade = PaymentFacade(afterPgProcessor, applicationEventPublisher)
    ...
    fun `executeAfterPg 성공 시 OrderStatusChangeDto 이벤트가 발행된다`() {
        ...
        every { afterPgProcessor.executeAfterPg(command) } returns PaymentStatus.SUCCESS

        // when
        paymentFacade.executeAfterPg(command)

        // then
        val slot = slot<OrderStatusChangeDto>()
        verify(exactly = 1) { applicationEventPublisher.publishEvent(capture(slot)) }
    ...
    fun `실패 시 이벤트에 CANCELLED 상태가 담겨 발행된다`() {
        ...
        every { afterPgProcessor.executeAfterPg(command) } returns PaymentStatus.FAILED
